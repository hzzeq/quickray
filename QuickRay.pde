...
void setup() {
  ...
  world.objects.add(new Sphere(new Vec3(0, 0, 3), 1)); world.objects.get(0).objColor = new Vec3(1, 1, 1);
...
void draw() {
  ...
  world.lights.get(0).position = new Vec3(cos((float)frameCount / 20) * 3, 1, 2);
  
  //c.position = new Vec3(sin((float)frameCount / 20) * 0.1, cos((float)frameCount / 8) * 0.1, 0);
  
  background(0);
    
  frame = c.renderImage(world, 500, 500);
